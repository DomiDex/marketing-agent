 ...
 */
export async function getSkillContent(skillName) {
    const prefix = `skills/${skillName}/`;
    // Get SKILL.md
    const skillMdResponse = await s3Client.send(new GetObjectCommand({
        Bucket: BUCKET,
        Key: `${prefix}SKILL.md`,
    }));
    ...
    let rulesMd;
    try {
        const rulesMdResponse = await s3Client.send(new GetObjectCommand({
            Bucket: BUCKET,
            Key: `${prefix}RULES.md`,
        }));
    ...
    if (listResponse.CommonPrefixes) {
        for (const commonPrefix of listResponse.CommonPrefixes) {
            ...
            if (subdirName && subdirName !== 'SKILL.md' && subdirName !== 'RULES.md') {
                ...
                const subdirResponse = await s3Client.send(new ListObjectsV2Command({
                    Bucket: BUCKET,
                ...
                if (subdirResponse.Contents) {
                    for (const obj of subdirResponse.Contents) {
                        ...
                        if (content)
                            subdirectories[subdirName][fileName] = content;
                    }
                }
            }
        }
